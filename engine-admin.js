 ...
 var serviceAccount = require("/opt/keys/firebase-account.json");
  ...
  .then(function () {
    ...
    requestsRef.on("child_added", function(snapshot, prevChildKey) {
     ...
     // var keywordRef = requestRef.child(snapshot.key+'/keyword');
      keywordRef.on("child_changed", function(snapshot2) {
      	
     keywordRef.once("value", function(snap) {
       console.log("changed data: ", snap.val());
              updateElastic(snap.val().value, snapshot.key);
    });
